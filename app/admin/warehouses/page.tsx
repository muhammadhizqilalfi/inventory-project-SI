    ...
    <div className="w-full p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">Kelola hierarki gudang, zona, dan rak penyimpanan.</p>
        </div>
        <AddWarehouseButton />
      </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        {warehouses.map((warehouse) => (
          <div key={warehouse.id} className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
            {/* Warehouse Header */}
            <div className="bg-gray-50 px-6 py-4 border-b border-gray-200 flex justify-between items-center">
              <div>
                <h2 className="text-lg font-bold text-gray-800">{warehouse.name}</h2>
                <p className="text-sm text-gray-500">{warehouse.location}</p>
              </div>
              <div className="flex gap-4 text-sm">
                <div className="text-center">
                  <span className="block font-bold text-gray-700">{warehouse._count.locations}</span>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="text-xs uppercase text-gray-400 bg-white border-b">
                    ...
                    <th className="px-6 py-3 font-semibold text-center">Total SKU</th>
                    <th className="px-6 py-3 font-semibold text-right">Aksi</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {warehouse.locations.map((loc) => (
                    <tr key={loc.id} className="hover:bg-blue-50 transition-colors group">
            ...
            <div className="p-4 bg-gray-50 border-t border-gray-100 text-right">
              <AddRackButton warehouseId={warehouse.id} warehouseName={warehouse.name} />
            </div>
          </div>
        ))}
      </div>
